...
"""
Demonstration of stepwise FMU_Handler simulation.
Stepwise FMU_Handler simulation is common for control tasks or co-simulation
...
A: System FMU_Handler and Python controller
B: System FMU_Handler and controller FMU_Handler

Exemplary, a thermal zone FMU_Handler model is used as system, that uses a control bus as interface. 
In case B, the controller FMU_Handler uses the same interface. 
...
During the stepwise simulation, two different input types can be applied onto an FMU_Handler: 
An input table with preassigned values (in this case bus.setPoint and bus.disturbance[1]) 
and input that is only valid for one step (bus.controlOutput, and in case of the controller FMU_Handler bus.processVar).
"""
...
# ------ Settings, input data, initial values, system FMU_Handler ---------------------------

# ---- Settings ---------
output_step = 60*10  # step size of simulation results in seconds (resolution of results data)
comm_step = 60/3  # step size of FMU_Handler communication in seconds (in this interval, values are set to or read from the fmu)
start = 0  # start time
...
# ---- System FMU_Handler ---- todo: move settings to FMU_Handler class and config file maybe
work_dir = pathlib.Path(__file__).parent.joinpath("results")
# path to fmu file  # todo: move settings to FMU_Handler class and config file maybe
path = pathlib.Path(__file__).parent.joinpath("data", "ThermalZone_bus.fmu")
# create FMU_Handler API object
sys = FMU_API_stepwise(fmu_path=path, cd=work_dir, log_fmu=False)  # Todo: allow path as expected type for model_name additionaly
# set custom simulation setup
sys.set_sim_setup(sim_setup=simulation_setup)  # Todo: Changed to property function in v0.1.7??


# ----- Study A: System FMU_Handler - Python controller ---------------------------------------------

# --- Initialize system FMU_Handler ------
sys.initialize_fmu_for_do_step(parameters={'T_start': t_start},
                               ...
                               store_input=True)  # default; the FMU_Handler inputs are added to the simulation results

# By default, the FMU_Handler in- and outputs are also added to the list of variables to read
...
# ------ Simulation Loop -------
# The central FMU_Handler functions used in the loop are read_variables_wr() and set_variables_wr()
# These functions extend the functionality of the basic FMU_Handler communication functions read_variables() and set_variables()
...
# 2. set_variables_wr writes the values in the input data table and inputs for the specific step to the FMU_Handler.
# Values from the input data table are either hold or interpolated.
# By default, a simulation step is performed after writing. Optionally, the FMU_Handler can be closed when stop time is reached.
while not sys.finished:
    # Read system state from FMU_Handler
    ...
    # write controller output to system FMU_Handler as well as pre-known inputs and perform step
    sys.set_variables_wr(input_step={'bus.controlOutput': ctr_action},
                         ...
                         automatic_close=False)  # default; the FMU_Handler is not closed when finished for second study
...
# ----- Study B: System FMU_Handler - Controller FMU_Handler ---------------------------------------------
...
# ------ Instantiate and initialize controller FMU_Handler-----
# path of fmu file  # todo: move settings to FMU_Handler class and config file maybe
path = pathlib.Path(__file__).parent.joinpath("data", "PI_1_bus.fmu")
ctr = FMU_API_stepwise(fmu_path=path, cd=work_dir, log_fmu=False)  # Todo: allow path as expected type for model_name additionaly
...
ctr.initialize_fmu_for_do_step(parameters=None,  # Not required for controller
                               ...
                               store_input=False)  # optional; the FMU_Handler inputs are not added to the results

# ------ Simulation Loop -------
while not sys.finished:
    # read system state from system FMU_Handler
...
ctr.close()  # controller FMU_Handler closed explicitly because it can not be closed in the while loop
...
for i in range(len(cases)):
    ...
    # title and y label
    if i == 0:
        axes[0].set_title('System FMU_Handler - Python controller')
        axes[0].set_ylabel('Zone temperature / °C')
        axes[1].set_ylabel('Rel. heating power / -')
        axes[2].set_ylabel('Ambient temperature / °C')
    if i == 1:
        axes[0].set_title('System FMU_Handler - Controller FMU_Handler')
